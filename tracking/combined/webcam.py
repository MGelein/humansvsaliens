...
def get_depth_map(frame, feed_width, feed_height):
...
with torch.no_grad():
    while True:
        ...
        bboxes = get_bounding_boxes(frame)
        depth_map = get_depth_map(frame, feed_width, feed_height)
        ...
        while bboxes[0, person_class_idx, j, 0] >= THRESHOLD:
            ...
            cv2.putText(frame, str(depth_map[int(pt[0])][int(pt[j])]), (int(pt[0]), int(pt[1])), FONT, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            j += 1
